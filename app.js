...
function copyAiInstallCommand(){navigator.clipboard?.writeText(AI_INSTALL_COMMAND).then(()=>{const notice=document.querySelector('#ai-install-notice');if(notice)notice.textContent='설치 명령을 복사했어요. PowerShell에 붙여 넣어 실행하세요.';}).catch(()=>{const notice=document.querySelector('#ai-install-notice');if(notice)notice.textContent='명령을 직접 선택해 복사해 주세요.';});}

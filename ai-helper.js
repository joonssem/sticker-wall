...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='POST'&&req.url==='/shutdown'){
      if(origin&&origin!==`http://${HOST}:${PORT}`)return send(res,403,{error:'로컬 설정 화면에서만 종료할 수 있습니다.'},origin);
      send(res,200,{ok:true},origin);
      return setTimeout(()=>server.close(()=>process.exit(0)),50);
    }
